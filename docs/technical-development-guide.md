...
- [MyPipelineHero CRM — Technical Development Guide](#mypipelinehero-crm--technical-development-guide)
  ...
  - [Part K — Deferred / Out-of-MVP Catalog](#part-k--deferred--out-of-mvp-catalog)
...
| --- | --- |
| M0 — Foundation | Docker compose, Django skeleton, root landing page, shared CSS assets, custom user model, seed-MVP, CI |
...
| M9 (post-MVP) — Phase 2 React Portal | DRF API surface, React tenant portal, Phase 1 retirement |
...
| **Capability** | A platform-defined permission code (e.g., `quotes.send`). Codes follow `{domain}.{resource}.{action}`. Tenants may not create custom codes in MVP. |
...
| **Engine Version** | The `<major>.<minor>` string identifying the pricing pipeline contract. MVP ships as `"1.0"`. |
...
### A.1 Product Vision and MVP Scope
...
**A.1.2 MVP scope is production-ready, not infrastructure-heavy**

MVP is a CRM plus a CPQ-capable pricing foundation. The pricing engine ***MUST*** be flexible enough to support services, resale products, manufactured products, bundles, recurring plans, discounts, approvals, tax, and immutable quote-time snapshots.

The MVP pricing architecture ***MUST NOT*** create one pricing strategy class for every named business scenario. Pricing behavior ***MUST*** be composed from:
...
Infrastructure ***MUST*** be production-ready but intentionally simple. MVP deploys without Kubernetes. Local development uses Docker Compose. Staging, demo, and production deploy to DigitalOcean using Docker images and Docker Compose or equivalent host-level container orchestration.
...
Every decision marked "deferred" or "future-friendly" MUST appear in K.1 with a one-line MVP accommodation note. "We'll figure it out later" is not a deferred decision; it's an implicit decision, and implicit decisions are prohibited.
...
**Worker tier.** Celery workers run in one or more separate containers. MVP MAY begin with one worker process consuming all queues. The worker configuration MUST preserve logical queue separation:
...
MVP deployment MUST use Docker images and environment-specific Docker Compose files or equivalent host-level container orchestration.

Kubernetes-specific resources are not part of MVP:
...
| Phase 1 domain templates | Every domain needs server-rendered pages for functional testing and production MVP usage before Phase 2 React parity |
...
The base Django admin is not the primary platform console and is not the tenant admin UI. It MAY be enabled in local development and selected non-production environments at `/django-admin/` for raw model inspection and framework debugging, but no MVP product workflow may depend on it. Production administration MUST use custom admin views.
...
Locked now: DRF as the API library, URL versioning (`/api/MVP/...`), drf-spectacular for OpenAPI schema, cursor pagination as default, cookie-based auth inheriting from tenant-local session.
...
This structure is the initial MVP layout. It MAY evolve as the codebase grows, but changes that move domain ownership, app boundaries, template ownership, static asset ownership, or frontend/backend boundaries MUST be reflected in this guide.
...
**Slug rules.** Slug MUST match `^[a-z][a-z0-9-]{1,61}[a-z0-9]$` (DNS-safe). Slug is unique globally. Slug is immutable after Organization creation in MVP.
...
DB-level CHECK constraints across tables NOT used in MVP.
...
**B.1.8 Tenant Onboarding Workflow (MVP)**

**Status: NORMATIVE.**

**B.1.8.1 Scope**

This section specifies the complete MVP path from "we have a new tenant" to "the tenant's owner is signed in and ready to use the product." Self-service signup is deferred (K.4); MVP onboarding is operator-mediated.
...
**B.1.8.3 Operator-initiated onboarding (MVP only path)**
...
4. Creates the default `InvoicingPolicy` with MVP default values.
...
- **Owner accepts invite, then immediately resigns.** Org Admin must invite another user with `admin.members.invite` capability and assign the Owner role manually. There is no "transfer ownership" workflow in MVP (deferred to K.4).

- **Slug squatting.** Slugs are global. If a competitor signs up with a slug similar to a known tenant, the platform operator must intervene. MVP does not provide an automatic dispute mechanism.

***B.1.8.6 Decisions embedded in this section***

- MVP onboarding is operator-mediated only; self-signup deferred to K.4.
...
Provider configuration is platform-managed in MVP.
...
External provider groups, domains, or claims MUST NOT grant application capabilities directly in MVP.

Mapping identity-provider groups to tenant roles is deferred.

**B.3.10 System User**

Exactly one `User` row per environment has `is_system=true`. Created via the seed-MVP data migration. Has unusable password. MUST NOT have external identities.
...
2FA/MFA is required at MVP.

For local password login:

- Local MFA is REQUIRED.
- TOTP is REQUIRED for MVP.
...
| OAuth/OIDC logout | Local logout only in MVP; provider logout not guaranteed |
...
| Reuse prevention | Not enforced in MVP |
...
MVP required local MFA methods:
...
This section is the authoritative entity inventory for MVP. Cross-references: Identity entities (User, Organization, Membership, Role, Capability) are defined in B.1, B.3, B.6. Operating-scope entities (Region, Market, Location, MembershipScopeAssignment) are defined in B.2.
...
WorkOrder
  ...
  hold_reason, cancelled_reason: TEXT, null
  recurrence_template_id: UUID, null              -- reserved; null in MVP
```
...
Number gaps occur when a transaction rolls back. Gaps are EXPECTED. Gap-free numbering is NOT supported in MVP.
...
5. **Cross-table org consistency** is NOT enforced via DB constraints in MVP.
...
The "one SalesOrderLine = one fulfillment artifact" rule is a MVP simplification. Quote builder shows a soft warning when:
...
Reminders dispatch outbound email only in MVP.
...
def send_communication(
    *, organization_id, actor_id,
    ...
    """ OUTBOUND EMAIL ONLY in MVP. Body immutable once sent. """
```
...
`malware_scan_status=PENDING` on upload. MVP: no-op scanner immediately marks SKIPPED. INFECTED attachments quarantined: download URLs return 403.
...
**MVP PriceLists are reached only via ClientContractPricing.** Standalone PriceList strategy is deferred.
...
E.5.3 MVP base strategies

The MVP base strategy registry contains exactly:
...
MVP ships as engine version `"1.0"`.
...
### E.6 Pricing Engine: Base Strategy Catalog (MVP)

**Status: NORMATIVE.**

This section defines the complete MVP base strategy catalog. Each strategy receives a complete `PricingContext`, performs deterministic math, and returns a `BasePricingResult`.
...
MVP supports quote-time recurring plan pricing only. Automated renewals, metered usage ingestion, and recurring invoice generation are deferred unless separately approved.
...
CI test `test_strategy_registry_complete` MUST assert that these are the only MVP base strategies unless the guide is amended.

### E.7 Pricing Engine: Modifier Catalog (MVP)
...
E.10.5 Mixed service/product line composition (MVP approach)

**Status: NORMATIVE.**

In MVP there is **no `QuoteGroup` entity**. Mixed service/product packages are expressed through the existing entity model in one of two ways.
...
| PART_RECEIVED | No (partial cancel out of MVP) |
...
- `WorkOrder.recurrence_template_id` — null in MVP; reserved for recurring service templates.
...
- Partial invoicing only at parent level in MVP.
...
A Payment with `unapplied_amount > 0` after allocations remains as unapplied credit. Visible on client's account. MAY be applied to future invoices via `allocate_payment`. NOT auto-applied. Refunds and write-offs out of MVP.
...
MVP ships per-jurisdictional tax. Entities: `TaxJurisdiction` (org-scoped, hierarchical), `TaxRate` (effective-dated), `Location.tax_jurisdiction_id`, `Organization.default_tax_jurisdiction_id`, `Client.tax_exempt`.
...
The Noop adapter is the default in MVP.
...
F.6.1 MVP fixed report catalog
...
The outbox dispatcher MAY run as a Celery beat-triggered task or as a long-running worker command. In MVP, the default implementation is a beat-triggered dispatcher.
...
OpenTelemetry SDK installed; metrics export configurable (no-op default in MVP).
...
OpenTelemetry tracing SDK installed. Spans automatically created for HTTP requests, Celery tasks, service-layer functions (via decorator), pricing pipeline invocations. Exporter MAY be no-op in MVP.
...
`AuditEvent.schema_version: INT, default(1)`. MVP ships at version 1. Major schema changes require version bump + migration helper.
...
MVP fields requiring field-level encryption:
...
G.7.1 Why this is in MVP

Per the locked answer in batch 1, tenant data export and deletion are required at MVP launch. Drives: GDPR/CCPA-equivalent obligations + clean exit story for external first-tenant CRM.
...
| Date/time picker | Native `<input type="date">` / `datetime-local` (no JS picker library in MVP) |
...
Reusable Django template partials live under `apps/web/templates/components/`. MVP registry:
...
`/django-admin/` is not a product surface. It MAY exist only for development and controlled non-production debugging. It MUST NOT be required for any MVP tenant, support, or platform workflow.
...
MVP dashboard intentionally minimal. Sections:
...
- **Pagination** — offset-based for MVP (cursor for Phase 2 API). 25 rows default.
...
- Dashboard scope intentionally minimal in MVP; KPI dashboards deferred (K.8).
- Bulk actions deferred. Per-row actions only in MVP.
...
4. **Capability-aware UI.** Capabilities loaded once per session via `GET /api/MVP/me/capabilities` and cached client-side. Hide-not-block.
...
### H.6 Internal API (DRF, MVP scope)

**Status: NORMATIVE.**

H.6.1 Posture and scope decision

MVP ships a **minimal, internal-only DRF API surface** sized to support exactly two consumers:
...
- API surfaces for every CRUD operation. MVP only exposes endpoints the Phase 1 UI consumes plus a small read-only catalog/pricing surface for M9 React development.
- Webhooks. Webhooks are K.11.
- API tokens for tenant-to-system integrations. Deferred to K.11.

This minimal surface preserves the architectural option to expand in M9 without forcing a "full API parity" investment in MVP.

H.6.2 Authentication

The internal API uses **session-cookie authentication** (the tenant-local session established via handoff per B.4.14). There is NO API token authentication in MVP.
...
```text
https://{slug}.mypipelinehero.com/api/MVP/...
```

The `MVP` segment is locked: breaking changes require `v2` and parallel deployment, not in-place evolution. MVP will run until M9 stabilizes; v2 plans are out of scope.

Routing lives in `backend/apps/api/MVP/urls.py`. Each domain owns a router that the MVP root includes.

H.6.4 MVP endpoint inventory

The following endpoints ship in MVP. Endpoints not in this list are NOT in scope.

**Read-only catalog & pricing (for React M9 dev and HTMX previews):**

```text
GET   /api/MVP/catalog/services/                   -- paginated list
GET   /api/MVP/catalog/services/{id}/              -- detail
GET   /api/MVP/catalog/products/                   -- paginated list
GET   /api/MVP/catalog/products/{id}/              -- detail
GET   /api/MVP/catalog/bundles/                    -- paginated list
GET   /api/MVP/catalog/bundles/{id}/               -- detail
GET   /api/MVP/catalog/suppliers/                  -- paginated list
GET   /api/MVP/pricing/preview/                    -- POST-able pricing preview;
...
```text
GET   /api/MVP/search/clients/?q=...               -- autocomplete; max 20 results
GET   /api/MVP/search/leads/?q=...                 -- autocomplete
GET   /api/MVP/search/services/?q=...
GET   /api/MVP/search/products/?q=...
GET   /api/MVP/search/locations/?q=...             -- operating-scope filtered
```

**Quote builder support (HTMX-consumed):**

```text
GET   /api/MVP/quotes/{id}/                        -- detail with versions and lines
POST  /api/MVP/quotes/{id}/versions/{vid}/preview-pricing/
...
```text
GET   /api/MVP/leads/                              -- paginated, filterable list
GET   /api/MVP/leads/{id}/                         -- detail
GET   /api/MVP/quotes/                             -- paginated list
GET   /api/MVP/clients/                            -- paginated list
GET   /api/MVP/clients/{id}/                       -- detail
GET   /api/MVP/orders/                             -- paginated list
GET   /api/MVP/orders/{id}/                        -- detail
GET   /api/MVP/invoices/                           -- paginated list
GET   /api/MVP/invoices/{id}/                      -- detail
```

**Write endpoints (limited; HTMX autosave and React-eventual-write):**

```text
POST   /api/MVP/leads/                             -- create
PATCH  /api/MVP/leads/{id}/                        -- partial update
POST   /api/MVP/quotes/{id}/versions/{vid}/lines/  -- add line (draft only)
PATCH  /api/MVP/quotes/{id}/versions/{vid}/lines/{lid}/
PATCH  /api/MVP/clients/{id}/                      -- partial update
```

All other write operations (quote send, accept, retract; payment recording; order cancellation; etc.) are **server-rendered Phase 1 form endpoints** in MVP, not API endpoints. The pattern is: HTMX submits the form to a Django view; the view calls the service; the response is an HTMX-friendly partial or redirect. This intentionally keeps complex multi-step workflows in the server-rendered tier where they're easier to reason about.
...
1. Live in `apps/<domain>/serializers.py` and be discovered by `apps/api/MVP/urls.py` explicitly (no `**kwargs` auto-discovery).
...
DRF Spectacular generates an OpenAPI 3 spec at `/api/MVP/schema/` (authenticated; tenant-portal only). A Swagger UI renders at `/api/MVP/docs/` for development tenants only (not production). Production OpenAPI access is for internal engineering only.

The OpenAPI spec is committed to the repo at `docs/api/MVP/openapi.yaml` and a CI job verifies the generated spec matches the committed spec on every PR. Drift fails CI.

H.6.11 Phase 2 expansion path

When React UI in M9 needs an endpoint not in the MVP inventory above, the expansion procedure is:

1. Add a guide-PR amending H.6.4 with the new endpoint.
2. Implement the endpoint in `apps/api/MVP/`.
...
There is no "MVP.x" or sub-versioning; the MVP surface grows by addition. Breaking changes wait for v2.
...
**Explicitly NOT MVP (and not blockers for M9 React work):**
...
- MVP ships a minimal internal API; not a full public API.
- Session-cookie auth only; no API tokens in MVP.
...
3. Use the same currency code across a single context (no mixed-currency contexts in MVP).
...
def test_committed_schema_matches_runtime():
    ...
    response = client.get("/api/MVP/schema/?format=json")
...
MVP deploys without Kubernetes.
...
I.4.13 Out-of-scope for MVP deployment
...
| --- | --- | --- |
| Platform seed | Capabilities, default roles, System User, default tax jurisdictions | Django data migration `seed_MVP.py` |
...
```python
# apps/platform/rbac/migrations/0002_seed_MVP.py
from django.db import migrations
from apps.platform.rbac.seeds.MVP_capabilities import MVP_CAPABILITIES
from apps.platform.rbac.seeds.MVP_default_roles import MVP_DEFAULT_ROLES
...
def seed_MVP(apps, schema_editor):
    ...
    for cap_def in MVP_CAPABILITIES:
    ...
    for role_def in MVP_DEFAULT_ROLES:
...
def unseed_MVP(apps, schema_editor):
    pass
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_MVP, unseed_MVP),
    ]
```
...
```python
# apps/platform/rbac/migrations/0017_seed_MVP_3_pricing_capabilities.py
def add_MVP_3_capabilities(apps, schema_editor):
    Capability = apps.get_model("platform_rbac", "Capability")
    Role = apps.get_model("platform_rbac", "Role")
    RoleCapability = apps.get_model("platform_rbac", "RoleCapability")
...
apps/platform/rbac/migrations/0002_seed_MVP.py              # depends on platform_accounts.0001 + platform_organizations.0001 + platform_rbac.0001
...
Part J is the binding roadmap for the MVP build. It is the contract between engineering and the business about what ships, in what order, with what guarantees. Every milestone has explicit, testable exit criteria; "done" is not a judgment call.
...
| I.6.3 | `seed_MVP` data migration (capabilities scaffold, default role templates scaffold, System User) |
| I.6.6 / B.1.8 | `services.create_organization` callable from the platform admin shell; MVP onboarding workflow per B.1.8 (operator-initiated) |
...
| 6 | One `is_system=True` user exists after `seed_MVP` runs | `pytest apps/platform/rbac/tests/seeds/test_system_user.py` |
...
| 10 | Base Django admin is not required for any MVP product workflow | review |
...
5. **Dev-only Django admin becomes a product dependency.** Mitigation: `/django-admin/` is documented as raw model inspection only, and exit criteria require no MVP workflow depends on it.
...
- django-allauth is the MVP authentication integration library.
...
Implement the three-layer authorization enforcement, the full MVP capability registry and default roles, MembershipCapabilityGrant override semantics, the AuditEvent table with monthly partitioning, the typed exception taxonomy, the outbox pattern with dispatcher and beat, and structured logging.

J.4.2 Scope

| Reference | Deliverable |
| --- | --- |
| B.6.3 | Full MVP capability registry seeded |
...
| 2 | All 11 default role templates exist in DB after `seed_MVP` | `pytest apps/platform/rbac/tests/seeds/test_MVP_default_roles.py` |
...
| E.10 | Engine version policy MVP.0 |
...
The strategy registry MUST include exactly these MVP base strategy codes unless the guide is amended:
...
J.5.6 Strategy classes explicitly prohibited in MVP

The following MUST NOT be implemented as standalone base strategy classes in MVP:
...
| 12 | `strategy.recurring_plan` supports MVP quote-time recurring plan pricing | deterministic pricing tests |
...
- MVP pricing uses 7 reusable base strategies.
...
- WeasyPrint locked as PDF rendering engine in MVP.
...
| H.7 | Custom admin workflow coverage for each MVP domain |
...
M8 makes the MVP build production-ready using the non-Kubernetes DigitalOcean/Docker deployment model.
...
- MVP production deployment is Docker-based on DigitalOcean.
- Kubernetes is deferred.
- OAuth/OIDC production readiness is part of launch readiness.
- django-allauth remains the MVP authentication integration layer.
...
### J.11 M9 — Phase 2 React Portal (post-MVP)

**Status: NORMATIVE for the contract; INFORMATIVE for sprint-level scoping.**

J.11.1 Goals

After MVP is stable in production, build the DRF-based internal API and the custom React tenant portal. Replace Phase 1 server-rendered tenant-portal screens domain-by-domain. Retain Phase 1 for login, account, platform console, support tooling, and email templates.

J.11.2 Scope

| Reference | Deliverable |
| --- | --- |
| H.6 | Full DRF internal API surface per H.6.4 (MVP baseline) plus expansion endpoints added under H.6.11 |
...
| # | Criterion | Verification |
| --- | --- | --- |
| 1 | DRF API exposes every endpoint in H.6.4 (and any post-MVP expansions per H.6.11) with documented capability requirements | OpenAPI schema review |
...
| 7 | Capability-aware UI loads capabilities once via `GET /api/MVP/me/capabilities` and caches for the session | manual |
...
**Entry:** M8 complete; MVP in production for at least 60 days with stable error rates.
**Blocks:** Public API (K.11) and any React-dependent integrations.

J.11.6 Suggested duration

**Ongoing post-MVP.** Initial DRF API + first domain cutover: 8–12 weeks. Subsequent domains: 2–4 weeks each. Phase 1 retirement when feature parity complete: 6–12 months elapsed.
...
- M9 may be deferred indefinitely (Phase 2 is post-MVP).
...
2. **Cut nice-to-have reports.** The 10-report catalog is the MVP bar; specific reports may slip to a post-MVP patch.
...
The "NEVER cut" list is the irreducible safety floor of MVP.
...
- M3 + M4 parallelism is the only sanctioned parallelism in MVP.
...
## Part K — Deferred / Out-of-MVP Catalog
...
Part K is the registry of every "we'll do that later" promise made in batches 1–5. It is the contract about what is explicitly NOT in MVP, why it isn't, what MVP already does to accommodate the future implementation, and what target version it lands in.
...
| **MVP accommodation** | What MVP already does to make a future implementation cheap |
| **Prerequisites** | Other K.1 items or MVP work that MUST land first |
...
| Motivation | Some tenants want public-facing price lists (catalog pricing) that apply to all clients, not contract-specific lists. MVP forces all PriceList use through ClientContractPricing. |
| MVP accommodation | The PriceList model and PriceListItem model already exist. Standalone public price lists can be implemented through `strategy.fixed_price` plus price-list resolver semantics rather than a new base strategy. |
| Prerequisites | None. |
| Affected sections | E.3.3, E.6 |
| Effort tier | S |
| Target version | MVP.5 |
...
| MVP accommodation | BundleComponent has FK to Service or Product but not to BundleDefinition. Adding the FK is a migration; the decomposition logic at acceptance (D.4.2) needs to recurse. |
| Prerequisites | None. |
...
| Motivation | Tenants with running promotions plus loyalty campaigns hit lines eligible for both; MVP picks only the highest-priority. |
| MVP accommodation | The promotion modifier (E.7.7) is the only place this logic lives. PromotionUsage tracks eligibility caps. |
| Prerequisites | None. |
...
| Motivation | Outcome-based pricing requires a deferred invoice for the success fee; MVP only invoices the base fee. |
| MVP accommodation | PricingSnapshot can retain outcome-related fields inside `base_inputs` if manually captured, but no value/outcome base strategy is registered in MVP. |
...
| Field | Value |
| --- | --- |
| Description | Tenants can define their own units of measure beyond the 21 MVP enum values; potentially with conversion factors. |
| Motivation | Specialized industries have unit conventions outside the MVP set. |
| MVP accommodation | The UoM field on catalog items and quote lines is a TEXT column, not a hard enum at the DB layer. The enum lives at the form/API boundary. |
...
| MVP accommodation | None directly. The CONFIGURABLE bundle pattern is similar but heavier. |
| Prerequisites | None. |
...
| MVP accommodation | The data model supports it; this is purely a UX/service layer addition. |
| Prerequisites | None. |
...
| Motivation | Real-world tenant operations require refunds; MVP's "out of scope" stance forces operators to issue refunds outside the system. |
| MVP accommodation | PaymentAdjustment (REVERSAL/CORRECTION) handles correction cases; PaymentAllocation reversal handles unwinding allocations. |
...
| Field | Value |
| --- | --- |
| Description | A first-class CreditBalance entity that tracks unapplied client credit, replacing MVP's `Payment.unapplied_amount` accumulation. |
| Motivation | Tracking credit across multiple Payments via `unapplied_amount` is workable for MVP but doesn't scale. |
| MVP accommodation | `Payment.unapplied_amount` is accessible via client account view. Migration path: a CreditBalance can be created with `source_payment_id` references for backwards compatibility. |
| Prerequisites | None. |
...
| MVP accommodation | None — MVP explicitly limits bundle invoicing to the parent line. |
...
| MVP accommodation | `manual_release_for_invoicing` (F.2.3) covers most cases. |
| Prerequisites | None. |
...
| MVP accommodation | The PO state machine prohibits this; manual reconciliation outside the system. |
| Prerequisites | None. |
...
| MVP accommodation | The User model has TOTP fields; adding WebAuthn credential records is purely additive. |
| Prerequisites | None. |
...
| MVP accommodation | None — MVP challenges 2FA on every login. |
| Prerequisites | None. |
...
| MVP accommodation | `services.create_organization` is the service-layer entry point; MVP only exposes it via the operator-mediated workflow in B.1.8 (platform admin console). |
...
| MVP accommodation | None. Each Client is currently flat. |
...
| MVP accommodation | `Organization.base_currency_code` is single; PricingSnapshot, Invoice, and SalesOrder carry `currency_code`; the currency modifier slot exists at pipeline step 22 but raises if `target_currency_code` set. |
...
| MVP accommodation | The PricingContext has `fx_rate` and `target_currency_code` fields reserved but unused. |
...
| MVP accommodation | TaxJurisdiction supports hierarchical jurisdictions; TaxRate supports per-line-type filtering; Client has `tax_exempt` flag. |
...
| MVP accommodation | `Client.tax_exempt_certificate_ref` is a TEXT field; DocumentAttachment domain exists. |
| Prerequisites | None. |
...
| MVP accommodation | The adapter Protocol, registry, NoopAccountingAdapter, and outbox-driven sync pattern all exist. |
| Prerequisites | Per-tenant adapter configuration UI. |
| Affected sections | F.5 |
| Effort tier | L per adapter |
| Target version | MVP.5 (QuickBooks first); v2 (Xero, NetSuite) |
...
| MVP accommodation | None — MVP sync is one-way. |
| Prerequisites | K.6.1. |
| Affected sections | F.5 |
| Effort tier | XL |
| Target version | v3+ |
...
| MVP accommodation | None. POs are emailed in MVP. |
...
| MVP accommodation | None — MVP explicitly excludes inventory. |
...
| MVP accommodation | `WorkOrder.recurrence_template_id` is reserved (null in MVP). |
| Prerequisites | None. |
...
| MVP accommodation | WorkOrder has `assigned_to_membership_id`, `scheduled_date`, `client_location_id`. Manual assignment in MVP. |
...
| MVP accommodation | None — MVP ships a fixed palette. |
...
| MVP accommodation | DocumentAttachment domain handles file uploads. |
| Prerequisites | None. |
...
| Motivation | The minimal MVP dashboard is functional but not differentiating. |
| MVP accommodation | The current dashboard is a single template; replacing it with a tile registry is straightforward. |
| Prerequisites | None. |
...
| Motivation | a11y is a baseline expectation; MVP ships with manual review only. |
| MVP accommodation | Form rendering through partials and progressive enhancement reduce the worst a11y risks. |
| Prerequisites | None. |
| Affected sections | I.1 |
| Effort tier | M |
| Target version | MVP.5 |
...
| MVP accommodation | PDFs are the canonical printable artifact. |
| Prerequisites | None. |
| Affected sections | H.4 |
| Effort tier | S |
| Target version | MVP.5 |
...
| Motivation | Tenants with reporting needs outside the MVP fixed catalog of 10. |
| MVP accommodation | The 10 fixed reports cover the ~80% case. |
...
| MVP accommodation | Reports run on demand only. |
| Prerequisites | None. |
...
| MVP accommodation | The `tenant.export.assemble` pipeline produces JSONL+CSV; that's a starting point. |
...
| Motivation | The minimal MVP dashboard doesn't help executives. |
| MVP accommodation | The 10 fixed reports cover the data; the dashboard can pull from them. |
...
| MVP accommodation | AuditEvent retention via partition detach-then-drop; partitions could be archived to object storage in another region. |
...
| MVP accommodation | Whole-tenant export covers the org-wide case but not per-user requests within an active tenant. |
| Prerequisites | None. |
...
| MVP accommodation | None — MVP's tenant isolation makes this deliberately hard. |
...
| MVP accommodation | The CSP report-uri, audit logging, and rate limits all help with disclosure forensics. |
...
| MVP accommodation | Application-tier security controls (CSP, rate limits, CSRF) are defense-in-depth without WAF. |
| Prerequisites | None. |
...
| MVP accommodation | None. |
| Prerequisites | Stable MVP in production. |
| Affected sections | G.6 |
| Effort tier | M (per cycle) |
| Target version | MVP.5 (first cycle) |
...
| MVP accommodation | The Communication model has direction=INBOUND and channel=EMAIL fields reserved; no inbound parser exists. |
...
| MVP accommodation | Communication has `provider_message_id` field; threading metadata could be added. |
...
| MVP accommodation | The outbox pattern emits notification events; in-app notifications would consume the same events via a different handler. |
| Prerequisites | None. |
...
| MVP accommodation | The forward-looking constraints in H.5.3 are framework-agnostic. |
| Prerequisites | MVP in production for 60+ days. |
...
| MVP accommodation | drf-spectacular generates the OpenAPI schema; CI validates it. |
| Prerequisites | None. |
...
| MVP accommodation | OpenAPI schema is committed; diffing is straightforward. |
...
| MVP accommodation | The internal API exists; making it public requires hardening (auth options, rate limits, SLAs, versioning policy, public docs). |
...
| MVP accommodation | The outbox pattern is the natural emission mechanism. |
...
| MVP accommodation | RollingUpdate strategy with `maxUnavailable: 0` provides zero-downtime, but no traffic-split canary. |
...
| MVP accommodation | Cross-region object-store replication is in MVP (I.5.3). Postgres replication to a remote region is the missing piece. |
| Prerequisites | Operational maturity at MVP launch + 6 months. |
...
| MVP accommodation | Manual failover via runbook. |
| Prerequisites | Production stability at MVP + 6 months. |
| Affected sections | I.4, I.5 |
| Effort tier | L |
| Target version | v2 |
...
| MVP accommodation | Whole-DB PITR exists; per-tenant restore requires logical-restore tooling. |
| Prerequisites | None. |
...
| MVP accommodation | Forward-only rollback via redeploying the prior SHA covers the common case. |
...
| MVP accommodation | Single staging environment. |
...
| MVP accommodation | All error messages have stable `error_code` strings; the codes are the source of truth. |
| Prerequisites | None. |
...
| MVP accommodation | Row-based tenancy with `organization_id` is fully implemented; schema-per-tenant would be a re-architecture. |
...
| MVP accommodation | None. New tenants enter records manually or via API. |
| Prerequisites | None. |
...
| MVP accommodation | None. The PricingSnapshot replay corpus is checked-in test data, not migration tooling. |
