...
- [Process History](#process-history)
  - [Part 1 — Document Reconciliation](#part-1--document-reconciliation)
  - [Part 2 — Conflict Resolution Confirmation](#part-2--conflict-resolution-confirmation)
  - [Part 3 — Outline Decisions](#part-3--outline-decisions)
- [Part FM — Front Matter](#part-fm--front-matter)
- [Part A — Foundations](#part-a--foundations)
  - [A.5 Base Project Structure](#a5-base-project-structure)
...
- [Part H - Frontend](#part-h--frontend)
  - [H.7 Custom Admin Sites and Development Inspection](#h7-custom-admin-sites-and-development-inspection)
  - [H.8 Design Assets and Phase 2 Style Parity](#h8-design-assets-and-phase-2-style-parity)
- [Consolidated Decisions Embedded](#consolidated-decisions-embedded)
- [Cross-Batch Propagation Notes](#cross-batch-propagation-notes)
...
# Consolidated Decisions Embedded

This appendix consolidates every "Decisions Embedded in This Section" callout from all expanded parts. Decisions are numbered sequentially across the guide and grouped by part for review. Override any item by referencing its number.

## Front Matter (FM) and Part A — Foundations (1–12)

| # | Section | Decision |
|---|---|---|
| 1 | FM.1 | Two-reviewer rule (eng + product/architecture) for guide PRs; drop to single-reviewer for INFORMATIVE if team is too small. |
| 2 | FM.1 | Versioning is `<major>.<minor>` (no patch). |
| 3 | FM.2 | Glossary is the authoritative term registry; new terms ship with their own glossary entry. |
| 4 | A.1 | v1 framed as "CRM + CPQ-class pricing engine" for staffing/timeline. |
| 5 | A.2 | Ten architectural principles; #9 (explicit over clever) and #10 (deferred decisions are documented) are additions. |
| 6 | A.3 | Four Celery queues (critical/default/bulk/reports). |
| 7 | A.3 | Exactly one Celery beat container per environment; harmful duplicate jobs require idempotency or Redis/Postgres locking. |
| 8 | A.3 | Object key prefix scheme `{environment}/orgs/{org_id}/{domain}/{record_id}/{filename}`. |
| 9 | A.3 | TLS terminates at Nginx/Caddy or a managed DigitalOcean/load-balancer certificate flow. |
| 10 | A.4 | Static AST check enforces service-layer discipline (CI-blocking). |
| 11 | A.4 | DRF locked over Django Ninja for Phase 2 internal API. |
| 12 | A.4 | HTMX as global default rather than case-by-case. |

## Part B — Tenancy, Identity, and Authorization (13–46)

| # | Section | Decision |
|---|---|---|
| 13 | B.1 | `TenantOwnedModel` abstract base class (not a mixin). |
| 14 | B.1 | `on_delete=PROTECT` on Organization FK (not CASCADE). |
| 15 | B.1 | `created_by`/`updated_by` nullable to accommodate System User. |
| 16 | B.1 | Slug immutable in v1. |
| 17 | B.2 | Location denormalized onto operational records (not joined through related). |
| 18 | B.2 | `Location.tax_jurisdiction_id` nullable with org-default fallback. |
| 19 | B.2 | Scoped role + no scope assignment = no data access. |
| 20 | B.3 | UUID v7 for User PK (uuid6 library). |
| 21 | B.3 | TOTP-only 2FA in v1; SMS rejected, WebAuthn deferred. |
| 22 | B.3 | TOTP secret encrypted at rest; backup codes one-way hashed. |
| 23 | B.3 | `is_default_for_user` enforced via partial unique index. |
| 24 | B.4 | 2FA mandatory for all users (not just admins). |
| 25 | B.4 | Handoff token NOT bound to user agent or IP (with documented mitigations). |
| 26 | B.4 | Sensitive-action re-auth uses TOTP only, not password. |
| 27 | B.4 | 5-minute sensitive-action window for most actions; 15-minute for admin. |
| 28 | B.4 | Tenant logout does NOT cascade to root or other tenant sessions. |
| 29 | B.4 | HMAC handoff signing key rotates quarterly; previous key retained for one rotation window. |
| 30 | B.4 | 7-day absolute session cap. |
| 31 | B.4 | Multi-tab UX is a soft warning interstitial, one-time per session pair. |
| 32 | B.5 | 12-char password minimum, no character classes; HIBP k-anonymity check on every set. |
| 33 | B.5 | 90-day rotation for admin/platform users only; no blanket rotation. |
| 34 | B.5 | No password reuse history in v1. |
| 35 | B.5 | Per-account lockout: 5/15min then 15-min lock; 10/1hr then 24-hr lock. |
| 36 | B.5 | Specific endpoint rate limits in B.5.5. |
| 37 | B.6 | Two new default roles: "Pricing Manager" and "Billing Staff." |
| 38 | B.6 | New-capability propagation auto-extends Owner only; other roles unchanged. |
| 39 | B.6 | DENY beats GRANT in MembershipCapabilityGrant. |
| 40 | B.6 | `Role.is_scoped_role` as explicit model flag. |
| 41 | B.6 | Capability-coverage CI test (every URL has decorator or explicit exemption). |
| 42 | B.6 | New pricing capabilities added: `pricing.rules.*`, `pricing.price_lists.manage`, `pricing.contracts.manage`, `pricing.labor_rates.manage`, `pricing.segments.manage`, `pricing.promotions.manage`, `pricing.bundles.manage`, `pricing.approval.request`, `pricing.approval.grant`, `tax.jurisdictions.manage`, `admin.export.request`, `admin.deletion.request`. |
| 43 | B.7 | 2-hour absolute cap on impersonation sessions. |
| 44 | B.7 | Impersonation banner server-rendered into base template; CI test asserts every tenant-portal template inherits base. |
| 45 | B.7 | Separate `ImpersonationAuditLog` table with 7-year fixed retention regardless of tenant lifecycle. |
| 46 | B.7 | Support-on-support impersonation prohibited. |

## Part C — Domain Model and State Machines (47–75)

| # | Section | Decision |
|---|---|---|
| 47 | C.1.1 | UUID v7 generated in Python via `uuid6` library (DB does not generate). |
| 48 | C.1.1 | Audit columns inherited from `TenantOwnedModel`; not repeated per-entity. |
| 49 | C.1.2 | `Lead.summary` is single TEXT field; activity log domain deferred. |
| 50 | C.1.2 | `LeadLocation` retained (rename to `LeadSite` flagged as cosmetic open question). |
| 51 | C.1.3 | `QuoteVersion.optimistic_version` integer for DRAFT-only concurrency control. |
| 52 | C.1.3 | `QuoteVersionLine.unit_price_snapshot` denormalized alongside `pricing_snapshot_id` FK. |
| 53 | C.1.4 | `Client.deleted_at` (hard delete for tenant offboarding) is distinct from `status=INACTIVE`. |
| 54 | C.1.9 | `BOM` container model (one per manufactured product) plus `BOMVersion`. |
| 55 | C.1.10 | `PricingSnapshot` and `AuditEvent` partitioned monthly from migration #1. |
| 56 | C.1.10 | `OutboxEntry.idempotency_key` with unique constraint per (topic, key). |
| 57 | C.1.10 | `PricingApproval.expires_at` defaults to 7 days from request. |
| 58 | C.1.11 | `TaxJurisdiction.parent_jurisdiction_id` permits hierarchical nesting (state→county→city). |
| 59 | C.1.11 | Multiple matching TaxRates SUM (snapshot records each component). |
| 60 | C.1.13 | `InvoicingPolicy` is a structured model (one per Org), not free-form JSON. |
| 61 | C.1.13 | `Payment` rows are NEVER updated; corrections only via `PaymentAdjustment`. |
| 62 | C.1.13 | `PaymentAllocation` reversals create new rows, not in-place mutation. |
| 63 | C.1.14 | `AuditEvent.event_category` enum to support category-aware retention pruning. |
| 64 | C.1.14 | `OutboxDeadLetter` table for inspection of permanently failed entries. |
| 65 | C.1.15 | `EntityNumberSequence` table with `SELECT FOR UPDATE` (not native Postgres sequences). |
| 66 | C.2.1 | Lead "Converted" is not terminal; lead may still be archived afterward. |
| 67 | C.2.10 | PricingApproval has `Withdrawn` terminal in addition to Approved/Rejected/Expired. |
| 68 | C.2.12 | Property-based test (Hypothesis) for state-machine completeness is a CI gate. |
| 69 | C.3.1 | Number format `{PREFIX}-{YEAR}-{SEQUENCE}`, sequence zero-padded to minimum 5 digits. |
| 70 | C.3.5 | Number gaps are EXPECTED; gap-free numbering NOT supported in v1. |
| 71 | C.4.1 | Status-based archival preferred over `deleted_at` where the entity has a meaningful lifecycle. |
| 72 | C.4.2 | Tenant deletion preserves `Organization` row as tombstone (status=DELETED) plus AuditEvent and ImpersonationAuditLog. |
| 73 | C.5.1 | Cross-table org consistency NOT enforced via DB constraints; service layer + tests cover it. |
| 74 | C.5.3 | Monthly partition granularity for AuditEvent/PricingSnapshot/OutboxEntry. |
| 75 | C.5.4 | Optimistic concurrency on QuoteVersion only; row-level locking elsewhere. |

## Part D — Commercial Workflow (76–98)

| # | Section | Decision |
|---|---|---|
| 76 | D.1.3 | Lead → Quote conversion does NOT copy notes or pre-populate quote lines. |
| 77 | D.1.3 | Lead → Quote conversion does NOT propagate lead sites; rep selects per-line if needed. |
| 78 | D.2.3 | Quote retraction RE-PRICES inherited lines (does not reuse retracted snapshots). |
| 79 | D.2.3 | Quote-level discount IS copied to the successor draft on retraction. |
| 80 | D.2.5 | Multi-quantity hint banner is dismissible per line; does NOT block save. |
| 81 | D.3.2 | Pricing snapshots are NOT mutated at acceptance; parent QuoteVersion status is the lock. |
| 82 | D.3.2 | Acceptance idempotency key is operator-provided (Phase 1 view derives from session+version hash). |
| 83 | D.3.3 | Lead → Client mapping does NOT auto-create ClientLocations from LeadLocations. |
| 84 | D.3.3 | `Client.billing_account_name` defaults to lead's primary contact full name; operator may edit. |
| 85 | D.3.4 | Client-resolution modal pre-selects neither tab; operator must choose deliberately. |
| 86 | D.4.2 | Bundle decomposition occurs at acceptance, not at quote time. |
| 87 | D.4.2 | New `SalesOrderLine.parent_sales_order_line_id` field for bundle children — C.1.5 amended. |
| 88 | D.4.3 | Resale lines do NOT auto-create POs; PO creation is operator-driven. |
| 89 | D.4.4 | SalesOrder status rollup is computed via service, not via DB triggers. |
| 90 | D.5.1 | Cancelling a SalesOrder requires ZERO Invoices (even VOID); void invoices first if needed. |
| 91 | D.6.2 | Client merge soft-deactivates the duplicate (not hard-delete); merge audit captures both IDs. |
| 92 | D.6.3 | Customer segment changes apply prospectively; historical PricingSnapshots unchanged. |
| 93 | D.7.3 | Task reminders dispatch via outbound email only in v1; in-app notifications deferred. |
| 94 | D.7.4 | Communication body immutability enforced via `body_hash`; no repair tool provided. |
| 95 | D.8.1 | Attachment max size 50MB in v1; tenant-customizable allowlist deferred. |
| 96 | D.8.4 | Malware scanner is a no-op in v1 with the interface and outbox path in place. |
| 97 | D.8.5 | `retention_until` defaults are document-kind-aware (7y for invoices/quotes, 3y for completion photos, 14d for export archives). |
| 98 | D.8.6 | `ATTACHMENT_ACCESSED` audit is sampled (1 in 10) to manage volume. |

## Part E — Catalog and Operations (99–139)

| # | Section | Decision |
|---|---|---|
| 99 | E.1.3 | 21 UoM enum values; tenant extension deferred. |
| 100 | E.1.3 | No UoM conversion in v1 (operators select consistent UoMs per item). |
| 101 | E.1.4 | `update_raw_material_cost` does NOT auto-supersede active BOMVersions. |
| 102 | E.2.2 | New BOM activation atomically supersedes any prior ACTIVE version. |
| 103 | E.2.3 | `cost_basis_at_creation` snapshotted at BOM ACTIVATION, not line creation. |
| 104 | E.2.4 | `BuildBOMSnapshot.snapshot_payload` stores fully denormalized JSON for replay independence. |
| 105 | E.3.2 | Specificity tuple ordering for rule resolution: item > client > segment > supplier > location > market > region > priority > created_at desc > id. |
| 106 | E.3.3 | PriceLists in v1 reached **only via ClientContractPricing**; standalone PriceList strategy deferred. |
| 107 | E.3.5 | LaborRateCard partial unique constraint where status=ACTIVE; **propagation to C.5.1**. |
| 108 | E.3.5 | Rate card resolution uses `occurred_on` for build labor, `requested_pricing_date` for quoting. |
| 109 | E.4.2 | New `PromotionUsage` model required for usage-cap enforcement; **propagation to C.1.10**. |
| 110 | E.4.2 | PromotionUsage written at quote SEND, not at acceptance; not removed on retraction (one-shot semantics). |
| 111 | E.4.3 | New `QuoteVersionLine.selected_options_json` field for CONFIGURABLE bundles; **propagation to C.1.3**. |
| 112 | E.4.3 | Bundle-of-bundles deferred; components reference Service or Product only. |
| 113 | E.5.2 | PricingContext is fully resolved by builder; strategies and modifiers receive only frozen primitives. |
| 114 | E.5.4 | Strategy registration is explicit dict; no autodiscovery or metaclass magic. |
| 115 | E.5.6 | Pipeline ordering frozen per engine_version; reordering = major bump. |
| 116 | E.5 | v1 pricing uses resolver → 7 base strategy → modifier pipeline → approval evaluation → tax/rounding → snapshot. |
| 117 | E.5.9 | `replay_pricing_snapshot` asserts recomputed total matches stored within 1-cent tolerance. |
| 118 | E.6 | Strategies that are also modifiers (segment, location, rush, etc.) ship as MODIFIERS first; base-strategy registration is a fallback. |
| 119 | E.6 | `strategy.value_outcome` and outcome-based success-fee billing are deferred unless explicitly added later. |
| 120 | E.7.9 | Quote-level discount allocated PROPORTIONALLY to line subtotal; residual cents to last line. |
| 121 | E.7.10 | Manual override DOES allow downstream modifiers (rounding, tax) to apply. |
| 122 | E.7.13 | Tax SUMMED across multiple jurisdiction components; each component recorded in modifier_log. |
| 123 | E.7.14 | Rounding uses banker's rounding (ROUND_HALF_EVEN) for nearest_cent and nearest_dollar. |
| 124 | E.7.15 | Currency modifier RAISES `PricingValidationError` if `target_currency_code` set in v1 (not silent no-op). |
| 125 | E.8 | Manual override and contract pricing are NOT PricingRules; they live in pipeline as modifier/strategy. |
| 126 | E.9.6 | Manual override ALWAYS triggers approval in v1; no per-tenant bypass. |
| 127 | E.9.6 | Re-pricing auto-withdraws pending PricingApproval with "Superseded by re-pricing" note. |
| 128 | E.9.6 | `request_pricing_approval` idempotent on (quote_version_line_id) for REQUESTED approvals. |
| 129 | E.9.6 | Approver notification via outbox email to all org members holding `pricing.approval.grant`; in-app notifications deferred. |
| 130 | E.10.1 | Snapshot JSON numerics stored as strings to preserve Decimal precision. |
| 131 | E.10.2 | Replay reconstructs context from snapshot JSON only — no DB lookups. |
| 132 | E.10.3 | Engine upgrade CI gate runs replay on a checked-in corpus of ~20 sample snapshots. |
| 133 | E.11.5 | PO cancellation prohibited at PART_RECEIVED; partial-cancel out of v1. |
| 134 | E.11.2 | Allocation sum invariant enforced at service layer, not DB. |
| 135 | E.12.1 | Build estimated costs sourced from PricingSnapshot at acceptance, not current catalog. |
| 136 | E.12.4 | Build variance computed read-side; never stored. |
| 137 | E.12.3 | Labor adjustment uses ORIGINAL entry's applied rate for delta, not current rate card. |
| 138 | E.13.3 | Outcome notes minimum 10 chars; new `CompletionValidationError` exception; **propagation to G.2**. |
| 139 | E.13.4 | Completion photos go through DocumentAttachment (no special photo table). |

## Part F — Billing (140–164)

| # | Section | Decision |
|---|---|---|
| 140 | F.1.1 | Invoice lines source pricing from SOL's snapshot, not recomputation. |
| 141 | F.1.2 | Partial invoicing scales snapshot pricing proportionally. |
| 142 | F.1.3 | Bundle invoicing presents parent line only; child components hidden from invoice presentation. |
| 143 | F.1.3 | Bundle partial invoicing only at parent line level; no per-component partial billing in v1. |
| 144 | F.1.5 | Invoice void requires zero non-reversed allocations; reverse first. |
| 145 | F.1.6 | Overdue marking is a beat job, not request-time computation. |
| 146 | F.2.4 | InvoicingPolicy changes apply prospectively; no retroactive re-evaluation. |
| 147 | F.2.6 | Bundle eligibility = ALL_COMPONENTS_ELIGIBLE by default. |
| 148 | F.2.6 | `NOT_INVOICEABLE` is a distinct enum value, not aliased to `INVOICED`. |
| 149 | F.3.7 | `Payment.amount` is NEVER mutated; PaymentAdjustment is the audit trail. |
| 150 | F.3.7 | Allocation reversal preserves original row with `reversed_at` flag (no negative-amount rows). |
| 151 | F.3.7 | Overpayment creates `unapplied_amount`; no automatic credit-balance entity in v1. |
| 152 | F.3.7 | Refunds and write-offs out of v1, period. |
| 153 | F.3.7 | `record_payment` idempotency key derived from session+timestamp+amount hash in Phase 1. |
| 154 | F.4.3 | Tax jurisdiction hierarchy walking depth bounded to 5. |
| 155 | F.4.3 | Recursive CTE for ancestor walking; cached per pipeline invocation only. |
| 156 | F.4.4 | Tax rates SUM, not multiply. |
| 157 | F.4.5 | `Client.tax_exempt` is a global override; tax = 0 if exempt OR not line.taxable. |
| 158 | F.5.3 | Accounting sync via outbox, never synchronous. |
| 159 | F.5.4 | `Organization.accounting_adapter_code` and `accounting_adapter_config` (encrypted at rest); **propagation to B.1.2**. |
| 160 | F.5.7 | v1 ships only NoopAccountingAdapter; concrete adapters deferred. |
| 161 | F.6.1 | v1 fixed report catalog of 10 reports; custom builders deferred. |
| 162 | F.6.2 | Synchronous threshold of 5000 rows; above goes async. |
| 163 | F.6.4 | 14-day export retention. |
| 164 | F.6.7 | Reports coded as Python functions, not stored procedures. |

## Part G — Cross-Cutting Concerns (165–200)

| # | Section | Decision |
|---|---|---|
| 165 | G.1.5 | One Python file per public service function. |
| 166 | G.1.7 | No DI container; module-level lookups + mock patching. |
| 167 | G.1.6 | Frozen `*Result` dataclasses for multi-entity returns. |
| 168 | G.2.3 | Stable `error_code` strings are public contract for Phase 2 React; renaming = breaking. |
| 169 | G.2.3 | 409 chosen for state/concurrency errors over 422. |
| 170 | G.2.5 | Phase 1 middleware redirects safe-state errors via messages framework; renders error page for others. |
| 171 | G.3.3 | Outbox dispatcher tick frequency = 5 seconds. |
| 172 | G.3.3 | `select_for_update(skip_locked=True)` for concurrent dispatcher safety. |
| 173 | G.3.4 | Idempotency keys include "revision" suffix for accounting-sync topics. |
| 174 | G.3.5 | Beat-driven housekeeping jobs run directly on Celery beat, not through outbox. |
| 175 | G.3.10 | Max attempts vary by topic (3 for cosmetic, 5 for critical, 10 for accounting). |
| 176 | G.3.10 | OutboxEntry CONSUMED retention = 30 days. |
| 177 | G.4.1 | Single-line JSON logs to stdout; cluster log sink handles routing. |
| 178 | G.4.2 | `correlation_id` propagated through outbox payload. |
| 179 | G.4.4 | Email addresses NOT redacted in logs (audit needs them); detailed masking is in audit (G.5.5). |
| 180 | G.4.7 | OpenTelemetry SDK installed in v1; metrics/traces exporters configurable (no-op default). |
| 181 | G.4.8 | Health endpoints unauthenticated. |
| 182 | G.4.9 | Initial alert thresholds in G.4.9; tuned post-launch. |
| 183 | G.5.3 | `audit_emit` MUST be called inside transaction.atomic(); enforced. |
| 184 | G.5.4 | Audit categorization derived from event_type prefix via lookup registry. |
| 185 | G.5.5 | Free-text fields in audit payloads truncated to 1000 chars + ellipsis. |
| 186 | G.5.6 | Retention pruning via partition detach-then-drop; no row-level DELETE. |
| 187 | G.5.7 | AuditEvent `schema_version` = 1 in v1; consumers handle variance. |
| 188 | G.5.8 | New capability `admin.audit.view` for tenant admin audit search; **propagation to B.6.3**. |
| 189 | G.6.4 | `SameSite=Lax` (not Strict) to accommodate handoff redirects. |
| 190 | G.6.5 | HTMX requests carry CSRF token via global JS init reading from cookie/hidden input. |
| 191 | G.6.6 | New rate limits: handoff endpoint (30/IP/min), Phase 2 API mutations (60/user/min), reads (600/user/min). |
| 192 | G.6.10 | Raw SQL allowed in `apps/reporting/exports/` with CI-tracked allowlist; prohibited elsewhere. |
| 193 | G.6.11 | Quarterly HMAC handoff-key rotation; annual project-level encryption key rotation. |
| 194 | G.7.2 | Export archive = gzipped tar with JSONL + CSV mirrors; attachments included at full size. |
| 195 | G.7.2 | 14-day retention on EXPORT_ARCHIVE attachment. |
| 196 | G.7.3 | 30-day grace period before tenant deletion executes. |
| 197 | G.7.3 | Confirmation phrase for tenant deletion = org slug verbatim. |
| 198 | G.7.3 | Tenant deletion preserves Organization tombstone (status=DELETED), AuditEvent, ImpersonationAuditLog. |
| 199 | G.7.3 | Tenant deletion executed by beat-driven worker, not human-driven button. |
| 200 | G.7.5 | New capabilities `admin.export.request`, `admin.deletion.request`; in B.6.3. |
