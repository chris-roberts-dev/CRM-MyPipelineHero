...
- The diagrams use Mermaid `erDiagram` syntax so they can render in GitHub, many Markdown editors, and documentation tools.
...
erDiagram
    ...
    ROLE {
      UUID id PK
      UUID organization_id FK_nullable
      string code
      boolean is_default
...
erDiagram
    ...
    LEAD {
      ...
      UUID organization_id FK
      UUID location_id FK
      string number UK_per_org
      string status
      UUID owner_membership_id FK_nullable
    }
    CLIENT {
      UUID id PK
      UUID organization_id FK
      ...
      string display_name
      string status
      UUID customer_segment_id FK_nullable
    }
    QUOTE {
      UUID id PK
      UUID organization_id FK
      UUID location_id FK
      string number UK_per_org
      UUID lead_id FK_nullable
      UUID client_id FK_nullable
    }
    QUOTE_VERSION {
      UUID id PK
      UUID quote_id FK
...
erDiagram
    ...
    SUPPLIER_PRODUCT {
      ...
      UUID raw_material_id FK_nullable
      decimal cost
    }
    BOM_VERSION {
      UUID id PK
    ...
    PRICING_SNAPSHOT {
      bigint id PK
      UUID organization_id FK
      UUID quote_version_line_id FK_nullable
      UUID invoice_line_id FK_nullable
...
erDiagram
    ...
    WORK_ORDER {
      UUID id PK
      UUID source_sales_order_line_id FK_UK
      UUID client_id FK
      UUID assigned_to_membership_id FK_nullable
      string status
    }
    PURCHASE_ORDER {
      UUID id PK
...
erDiagram
    ...
    OUTBOX_ENTRY {
      ...
      UUID organization_id FK_nullable
      string topic
      string idempotency_key UK_per_topic
      string status
    }
...
erDiagram
    ...
    TENANT_IMPORT_BATCH {
      UUID id PK
      UUID organization_id FK
      UUID onboarding_project_id FK_nullable
      string import_type
      string status
      string storage_key
      int row_count
